...
<head>
    <meta charset="UTF-8">
    <title>Update Combo: ${combo.comboName}</title>
...
<body>
    ...
    <div class="main-content">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="fw-bold mb-0">Update Combo</h2>
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 15px;">
            <form id="comboForm" action="${pageContext.request.contextPath}/combos/update" method="POST" enctype="multipart/form-data">
                <div class="card-body p-5">
                    <div class="row g-5">
                        ...
                        <div class="col-md-4 border-end">
                            <div class="mb-4">
                                ...
                                <input type="text" name="comboId" class="form-control input-custom bg-light" value="${combo.comboId}" readonly>
                            </div>
                            <div class="mb-4">
                                                                                ...
                                                                                <a class="dropdown-item d-flex justify-content-between align-items-center py-2 rounded-2"
                                                                                   ...
                                                                                   onclick="addProductToCombo('${p.productId}', '${p.productName}', ${p.price})">
                                                                                    <div class="d-flex flex-column">
                                                                                        ...
                                                                                        <span class="text-muted" style="font-size: 11px;">ID: ${p.productId}</span>
                                                                                    </div>
                            ...
                            <div class="selected-products-wrapper mt-3">
                                <div id="selectedProductsList">
                                    <%-- JS sẽ tự động đổ dữ liệu vào đây thông qua mảng ban đầu --%>
                                </div>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <div class="status-option-container">
                                    <div class="status-item">
                                        <input type="radio" name="statusCombo" id="st-pending" value="PENDING_APPROVAL" ${combo.statusCombo == 'PENDING_APPROVAL' ? 'checked' : ''}>
                                        <label for="st-pending">Pending Approval</label>
                                    </div>
                                    <div class="status-item">
                                        <input type="radio" name="statusCombo" id="st-active" value="ACTIVE" ${combo.statusCombo == 'ACTIVE' ? 'checked' : ''}>
                                        <label for="st-active">Active</label>
                                    </div>
                                    <div class="status-item">
                                        <input type="radio" name="statusCombo" id="st-discontinued" value="DISCONTINUED" ${combo.statusCombo == 'DISCONTINUED' ? 'checked' : ''}>
                                    ...
                                    <div class="inner-box shadow-sm" id="previewContainer">
                                        <c:choose>
                                            <c:when test="${not empty combo.imageUrl}">
                                                <img src="${combo.imageUrl}" style="width: 100%; height: 100%; object-fit: cover; border-radius: 10px;">
                                            </c:when>
                                            <c:otherwise>
                    ...
                    <div class="d-flex justify-content-end gap-3 mt-5 pt-4 border-top">
                        ...
                        <button type="submit" class="btn btn-primary px-5 py-2 fw-bold">Update Combo</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <%-- Khởi tạo dữ liệu sản phẩm cũ cho Update --%>
    <div id="combo-data-bridge"
         data-is-update="true"
         data-details='[
            <c:forEach var="detail" items="${combo.comboDetails}" varStatus="status">
                {
